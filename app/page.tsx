  ...
  useEffect(() => {
    ...
    Promise.all([
      getPostsByTag("uni_projects").then((d) =>
        setDbUniProjects(d as unknown as Post[]),
      ),
      getPostsByTag("personal_projects").then((d) =>
        setDbPersonalProjects(d as unknown as Post[]),
      ),
      getPostsByTag("it_events").then((d) =>
        setDbItEvents(d as unknown as Post[]),
      ),
      getPostsByTag("other_events").then((d) =>
        setDbOtherEvents(d as unknown as Post[]),
      ),
      getPostsByTag("lang_certs").then((d) =>
        setDbLangCerts(d as unknown as Post[]),
      ),
      getPostsByTag("tech_certs").then((d) =>
        setDbTechCerts(d as unknown as Post[]),
      ),
      getPostsByTag("other_certs").then((d) =>
        setDbOtherCerts(d as unknown as Post[]),
      ),
      getPostsByTag("achievements").then((d) =>
        setDbAchievements(d as unknown as Post[]),
      ),
      getAllPosts().then(
        (d) => d && setLatestPosts(d.slice(0, 3) as unknown as Post[]),
      ),
      Promise.all(
        [
          "about",
        ...
        if (secs.global_config)
          try {
            setGlobalConfig(JSON.parse(secs.global_config.contentEn));
          } catch {}
      }),
    ]).finally(() => {
