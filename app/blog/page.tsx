...
export default function BlogPage() {
  ...
  useEffect(() => {
    ...
    if (
      savedLang &&
      ["en", "vi", "jp"].includes(savedLang) &&
      savedLang !== currentLang
    ) {
      setCurrentLang(savedLang);
    }

    Promise.all([getAllPosts(), getSectionContent("global_config")]).then(
      ([data, configData]) => {
        if (data) setPosts(data as unknown as Post[]);
        if (configData) {
          try {
            setGlobalConfig(JSON.parse(configData.contentEn));
          } catch {}
        }
  ...
  const getTrans = (key: string) => {
    if (currentLang === "vi") {
      ...
      if (key === "no_result")
        return `Không tìm thấy bài viết nào phù hợp với "${search}" 🍃`;
      ...
      if (key === "no_result")
        return `「${search}」に一致する記事は見つかりませんでした 🍃`;
  ...
  const getCover = (json: string) => {
    try {
      ...
      return arr.length > 0 && arr[0]
        ? arr[0]
        : "https://placehold.co/600x400/ffc0cb/ffffff?text=Sakura+Blog";
    } catch {
      return "https://placehold.co/600x400/ffc0cb/ffffff?text=Sakura+Blog";
    }
  };
                ...
                style={{
                  ...
                  boxShadow:
                    selectedTag === tag.value
                      ...
                      : "none",
                  transform:
                    selectedTag === tag.value ? "scale(1.05)" : "scale(1)",
                  fontFamily: "inherit",
                }}
              >
                {tag.label}
          ...
          >
            <div
              style={{
                ...
                animation: "spin-slow 3s infinite",
                marginRight: "10px",
              }}
            >
              🌸
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          {ALL_TAGS.find((t) => t.value === post.tag)?.label ||
                            post.tag}
                        </span>
                        <h3
                          style={{
