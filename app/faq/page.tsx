...
export default function SakuraFaqPage() {
  ...
  const handleSendMail = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!token) {
      setSendMsg({
        text: 'Vui lòng tick vào ô "I am not a robot" 🌸',
        type: "error",
      });
      return;
    }
    ...
    if (file && file.size > 4 * 1024 * 1024) {
      // Tối đa 4MB
      setSendMsg({
        text: "File đính kèm quá lớn (Tối đa 4MB) 🍃",
        type: "error",
      });
      setIsSending(false);
      return;
    ...
    try {
      ...
      if (res.ok) {
        setSendMsg({
          text: "Đã gửi thư thành công! Hãy kiểm tra hộp thư của bạn nhé. 🌸",
          type: "success",
        });
        ...
        setSendMsg({
          text: errorData.error || "Có lỗi xảy ra khi gửi thư. 🍃",
          type: "error",
        });
          ...
          >
            ...
            <div>
              ...
              <textarea
                ...
                required
                rows={5}
                placeholder="Nhập câu hỏi chi tiết của bạn tại đây..."
                style={{
                  width: "100%",
                  padding: "15px",
                  borderRadius: "15px",
